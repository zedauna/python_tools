...
"""
# ---------------------------------------------------------------------------
# Name : script_listing_files.py (python 2.x)
# Creation: 13/04/2022
...
def scanner_rep(path,list_extension):
    ...
    if os.path.exists(path):
        ...
        text='Le repertoire '+path+'  compte : '+str(len(list_fitre))+' fichiers( l\'extension :'+str(' , '.join(list_extension))+' )\n'
        print_text(text)
        time.sleep(1)
        return list_fitre
    else:
...
if __name__ == '__main__':
    ...
    #Paramètres   
    path=raw_input('Repertoire à scanner : ')
    if path=='':
    ...
    ext=raw_input('Extension à scanner ( example : .mxd,.txt ) : ')
    if ext=='':
        list_extension=['.mxd']
    else:
        list_extension=list(ext.split(','))

    unit=raw_input('Unité de calcul : KB or MB or GB : ')
    if unit=='':
        unit='MB'
    ...
    if l!=0:
        print_text('Le repertoire '+path+'  compte : '+str(l)+'\n')
        time.sleep(1)
        ...
        print('Le repertoire '+path+'  compte : '+str(0)+'\n')
        myText.close()
        flog.write('Le repertoire '+path+'  compte : '+str(0)+'\n')

    #CSV
    myText.close()

    #LOG et Fin du programme
    print_text("\nFin traitement \n")
